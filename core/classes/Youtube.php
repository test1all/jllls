...

{
    ...
    function Get_Domain_Name($input,$www = 0)
    {
        $input = trim($input, '/');
    ...
    function search($q,$token = NULL,$limit = 100,$filter = NULL){
        ...
        $json = json_decode($int_data,true);
    ...
    && json_last_error() !== JSON_ERROR_NONE) {
        ...
        foreach ($listing as $dataz) {
            ...
            if(isset($dataz['videoWithContextRenderer']['videoId'])){

                

                $duration = $this->covertime(@$dataz['videoWithContextRenderer']['lengthText']['runs'][0]['text']);

                $parsed = date_parse($duration);

            @$data[$k]['id'] .= $dataz['videoWithContextRenderer']['videoId'];

            @$data[$k]['title'] .= $this->cleanText($dataz['videoWithContextRenderer']['title']['runs'][0]['text']); 

            if( empty($data[$k]['title']) )
            {
                $data[$k]['title'] = $this->cleanText(@$dataz['videoWithContextRenderer']['headline']['runs'][0]['text']); 
            }

            @$data[$k]['duration'] .= $dataz['videoWithContextRenderer']['lengthText']['runs'][0]['text'];
            ...
            @$data[$k]['channel'] .= $dataz['videoWithContextRenderer']['shortBylineText']['runs'][0]['text'];

            @$data[$k]['view'] .= $dataz['videoWithContextRenderer']['viewCountText']['runs'][0]['text'];

            @$data[$k]['nonapi'] .= 'yes';

            @$data[$k]['type'] .= 'video';
            @$data[$k]['image'] = 'https://img.youtube.com/vi/'.$dataz['videoWithContextRenderer']['videoId'].'/0.jpg';
